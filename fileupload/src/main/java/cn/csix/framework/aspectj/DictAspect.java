...
import cn.csix.common.utils.oConvertUtils;
import cn.csix.framework.aspectj.annotation.Dict;
...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * cn.csix.project..*.*Controller.*(..))")
    ...
    /**
     * 本方法针对返回对象为TableDataInfo的分页列表数据进行动态字典注入
     * 字典注入实现 通过对实体类添加注解@dict 来标识需要的字典内容,字典分为单字典code即可 ，table字典 code table text配合使用与字典的用法相同
